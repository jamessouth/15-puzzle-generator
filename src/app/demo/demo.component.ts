...
export class DemoComponent implements OnInit, OnDestroy {
  ...
  swapTiles(x, y) {
    ...
    this.ctx.drawImage(this.pic, this.canvArray[brdInd][0], this.canvArray[brdInd][1], 82, 82,
      this.canvArray[blank][0], this.canvArray[blank][1], 82, 82);
  ...
  useCanvas(arr, gameFinished) {
    this.pic.onload = () => {
      for (let i = 0; i < this.canvArray.length - gameFinished; i++) {
        this.ctx.drawImage(this.pic, this.canvArray[i][0], this.canvArray[i][1], 82, 82,
